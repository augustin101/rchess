...
//
// These are the canonical fixed constants from the Polyglot opening book format
// (Fabien Letouzey, ~2004). Every compliant implementation must use exactly
// these values.  Indices:
//   [0..767]  piece-square: (2 * piece_kind + color) * 64 + square
//             piece_kind: Pawn=0, Knight=1, Bishop=2, Rook=3, Queen=4, King=5
//             color: Black=0, White=1   (note: opposite of our Color repr)
...
//   [780]     side to move (XOR'd when it is White to move)
...
/// Compute the Polyglot Zobrist key for `board`.
///
/// This is NOT the same as `board.hash`, which uses the engine's own random
/// numbers.  This function uses the fixed Polyglot constants so the result
/// matches any compliant opening book.
pub fn polyglot_hash(board: &Board) -> u64 {
    let mut key = 0u64;

    // Pieces
    for sq in 0u8..64 {
        if let Some(piece) = board.mailbox[sq as usize] {
            let kind  = piece.piece_type as usize; // Pawn=0 … King=5
            let color = match piece.color {         // Polyglot: White=1, Black=0
                Color::White => 1,
                Color::Black => 0,
            };
            key ^= POLYGLOT_RANDOM[(2 * kind + color) * 64 + sq as usize];
        }
    }

    // Castling rights
    let cr = board.castling_rights;
    if cr.has(CastlingRights::WHITE_KINGSIDE)  { key ^= POLYGLOT_RANDOM[768]; }
    if cr.has(CastlingRights::WHITE_QUEENSIDE) { key ^= POLYGLOT_RANDOM[769]; }
    if cr.has(CastlingRights::BLACK_KINGSIDE)  { key ^= POLYGLOT_RANDOM[770]; }
    if cr.has(CastlingRights::BLACK_QUEENSIDE) { key ^= POLYGLOT_RANDOM[771]; }

    // En passant — only hash the file if a pawn can actually capture there.
    // Polyglot omits the ep key for "phantom" ep squares where no capture is
    // possible; failing to do this causes hash mismatches in many positions.
    if let Some(ep_sq) = board.en_passant {
        if ep_captor_exists(board, ep_sq) {
            key ^= POLYGLOT_RANDOM[772 + ep_sq.file() as usize];
        }
    }

    // Side to move: Polyglot XORs the turn key when it is WHITE to move.
...
/// Returns true if the side to move has a pawn that can capture on `ep_sq`.
fn ep_captor_exists(board: &Board, ep_sq: Square) -> bool {
    // pawn_attacks(opponent, ep_sq) gives the squares from which a pawn of
    // side-to-move would attack ep_sq.
    let their_color = board.side_to_move.flip();
    let attacker_squares = pawn_attacks(their_color, ep_sq);
    let our_pawns = board.pieces[board.side_to_move as usize][PieceType::Pawn as usize];
    attacker_squares & our_pawns != 0
}

// ── Book entry & file parsing ─────────────────────────────────────────────────
...
/// An in-memory Polyglot opening book.
...
impl OpeningBook {
    ...
    pub fn load(path: impl AsRef<Path>) -> Result<Self, std::io::Error> {
        ...
        for i in 0..n {
            let base  = i * 16;
            let key   = u64::from_be_bytes(data[base..base+8].try_into().unwrap());
            let poly_move = u16::from_be_bytes(data[base+8..base+10].try_into().unwrap());
            let weight = u16::from_be_bytes(data[base+10..base+12].try_into().unwrap());
            entries.push(BookEntry { key, poly_move, weight });
        }

        Ok(OpeningBook { entries })
    }

    /// Load the bundled high-quality book (`opening_books/Elo2400.bin`).
    pub fn load_default() -> Self {
        let bytes = include_bytes!("opening_books/Elo2400.bin");
        let n     = bytes.len() / 16;
        let mut entries = Vec::with_capacity(n);
        for i in 0..n {
            let base = i * 16;
            let key  = u64::from_be_bytes(bytes[base..base+8].try_into().unwrap());
            let poly_move = u16::from_be_bytes(bytes[base+8..base+10].try_into().unwrap());
            let weight = u16::from_be_bytes(bytes[base+10..base+12].try_into().unwrap());
            entries.push(BookEntry { key, poly_move, weight });
        }
        OpeningBook { entries }
    }
...
// Polyglot move bits:
//   0-2   to_file   (a=0 … h=7)
//   3-5   to_rank   (rank1=0 … rank8=7)
//   6-8   from_file
//   9-11  from_rank
//   12-14 promotion (0=none, 1=knight, 2=bishop, 3=rook, 4=queen)

fn decode_move(board: &Board, poly: u16) -> Option<Move> {
    let to_file   = ( poly        & 0x7) as u8;
    let to_rank   = ((poly >>  3) & 0x7) as u8;
    let from_file = ((poly >>  6) & 0x7) as u8;
    let from_rank = ((poly >>  9) & 0x7) as u8;
    let promo     =  (poly >> 12) & 0x7;
...
mod tests {
    ...
    fn polyglot_hash_starting_position() {
        ...
        assert_eq!(
            polyglot_hash(&board),
            0x463B96181691FC9C,
            "Polyglot hash of starting position does not match the canonical value"
        );
    }

    #[test]
    fn default_book_probes_starting_position() {
        let book  = OpeningBook::load_default();
        let board = Board::starting_position();
        let mv    = book.probe(&board).expect("starting position must be in the book");
        // The engine should return a sensible first move (e2e4, d2d4, etc.)
        let mv_str = mv.to_string();
        let known_first_moves = ["e2e4", "d2d4", "g1f3", "c2c4", "g2g3"];
        assert!(
    ...
    fn polyglot_hash_after_e4() {
        // After 1.e4, no en passant captor exists (Black has no d-file/f-file pawn
        // on rank 4 yet), so the ep square should NOT be hashed.
        let board = Board::from_fen(
            "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1",
        ).unwrap();
        let book  = OpeningBook::load_default();
        // Just verify hash is consistent and the book can be probed without panic.
        let _h = polyglot_hash(&board);
        let _  = book.probe(&board);
    }
}
